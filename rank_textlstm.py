...
from neural_ir.models.text_lstm import TextLSTM # import the TextLSTM model
...
parser = argparse.ArgumentParser(description="Ranking with TextLSTM")
parser.add_argument(
...
parser.add_argument(
    "--checkpoint",
    default="output/text_lstm/model",
    type=str,
...
parser.add_argument(
    "--o",
    type=str,
    default="output/text_lstm/test_run.trec",
...
model = TextLSTM.from_pretrained(args.checkpoint).to(args.device) # load the TextLSTM model
model.eval()
query_embs = []
docs_embs = []
doc_ids = []
for idx in tqdm(
    ...
    with torch.cuda.amp.autocast(), torch.no_grad():
        batch_embs = model.encode(**docs_inps).to("cpu") # use the encode method of TextLSTM
...
for idx in tqdm(
    ...
    with torch.cuda.amp.autocast(), torch.no_grad():
        batch_query_embs = (
            model.encode(**query_inps).to("cpu").numpy().astype("float32") # use the encode method of TextLSTM
        )
